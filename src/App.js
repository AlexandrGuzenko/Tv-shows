...
/*
          ...
          как предотвратить дальнейшее перемещение по страницам, если на этих страницах уже нет контента (неужели надо иметь своё свойство в стэйте на каждую страницу?)
          При поиске по названию не работают page и limit, да и вообще поиск по названию очень криво работает
          Если у меня один из компонентов требует перерисовки, но изменение нет в стэйте (например добавление класса), то надо это заносить в стэйт отдельным свойством того объекта, который собирают перерисовать?
у меня есть функция-конструктор url и потом собранный url я отправляю в fetch
Что-то вроде такого адреса
*/
